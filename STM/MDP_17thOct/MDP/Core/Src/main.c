...
CmdConfig cfgs[19] = {
		{0,0,SERVO_CENTER,0, DIR_FORWARD}, // STOP
		  {1556.6, 1910.7, SERVO_CENTER, 0, DIR_FORWARD}, // FW00
		  {2086.6, 2239.7, SERVO_CENTER, 0, DIR_BACKWARD}, // BW00 //{1796.4, 1854.2, SERVO_CENTER, 0, DIR_BACKWARD}, // BW00
		...
		//  {2700, 1600, 92, 89, DIR_BACKWARD}, // BR00,
		  ...
		  {150, 2100, 44, 85.25, DIR_FORWARD}, // fl00
		  {1630, 120.55, 117 ,-88, DIR_FORWARD}, // FR00 // -84.9 //-84.78 //over
		  {165
				  ...
				  , 2400, 45, -85.75, DIR_BACKWARD}, // BL00 // -86.1 //86.08 //over
		  {1650, 130, 110, 88.6, DIR_BACKWARD}, // BR00, // 88.9 //under //TIGHTEN //1700,160.55
...
//		__ADD_COMMAND(cQueue, 0, 0); // stop
	...
	else if (aRxBuffer[0] == 'F' && aRxBuffer[1] == 'T') __ADD_COMMAND(cQueue, 75, val); //FT00 fastestpathtask_V3
	...
	uint8_t message1[20];
	sprintf(message1, "cmd:%c%c%c%c", aRxBuffer[0], aRxBuffer[1], aRxBuffer[2], aRxBuffer[3]);
...
void RobotMoveDistObstacle(float * targetDist, const uint8_t speedMode) {
  ...
  do {
    ...
    if (abs(*targetDist - Distance) < 0.5) break;
...
//      speedScale = 1;
      ...
      if (difftime(time_now, time_started) > 10) break;
    }
...
// nothing to object_right
void Overshoot_right(int XX)
{
	uint16_t dataPoint_R = 0;
	uint32_t IR_data_raw_acc_R = 0;
	float obsDist_IR_R = 1000;
...
//	char XX_str[2];
//	sprintf(XX_str,"%s%s",aRxBuffer[2],aRxBuffer[3]);
//	int XX = atoi(XX_str);

	while ((obsDist_IR_R > dist_to_obj) || (dist_to_obj <= XX)){
		__ADC_Read_Dist(&hadc2, dataPoint_R, IR_data_raw_acc_R, obsDist_IR_R, obsTick_IR_R);
		__SET_MOTOR_DIRECTION(1);
		__SET_MOTOR_DUTY(&htim8, 1000, 2000);
		//	HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_1); // Used to make backwheel motors PWM channel is online
		//HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_2);
	}
	motorStop();
}

//nothing to object left
void Overshoot_left(int XX)
{
	uint16_t dataPoint_L = 0;
	uint32_t IR_data_raw_acc_L = 0;
	float obsDist_IR_L = 1000;
...
//	int XX = atoi(XX_str);
	...
	while ((obsDist_IR_L > dist_to_obj) || (dist_to_obj <= XX)){
		__ADC_Read_Dist(&hadc1, dataPoint_L, IR_data_raw_acc_L, obsDist_IR_L, obsTick_IR_L);
		__SET_MOTOR_DIRECTION(1);
		__SET_MOTOR_DUTY(&htim8, 1000, 2000);
	}
...
// object to nothing
void Not_Overshoot_right(int XX)
{
	uint16_t dataPoint_R = 0;
	uint32_t IR_data_raw_acc_R = 0;
	float obsDist_IR_R = dist_to_obj - 10;
	uint16_t obsTick_IR_R = 0;
...
// object to nothing
void Not_Overshoot_left(int XX)
{
	uint16_t dataPoint_L = 0;
	uint32_t IR_data_raw_acc_L = 0;
	float obsDist_IR_L = dist_to_obj - 10;
	uint16_t obsTick_IR_L = 0;
	  ...
	  if (curTask != TASK_FASTESTPATH_V2) osDelay(1000);
		  ...
		  switch (step){
		  	  case 1:
		  		  ...
		  		  if (__COMMAND_QUEUE_IS_EMPTY(cQueue)) {
		  				__CLEAR_CURCMD(curCmd);
		  				__OBJA_TASK_DONE(&huart3, rxMsg);
		  			}
		  		  curTask = TASK_NONE;
		  		  break;
		  	  case 2:
				sprintf(message1, "cse2: %d %c", debugCounter++, direction);
				OLED_ShowString(0, 50, message1);
		  		  if (direction == 'L'){
		  			__SET_CMD_CONFIG(cfgs[CONFIG_FL00], &htim8, &htim1, targetAngle);
		  			RobotTurn(&targetAngle);
		  			osDelay(10);
