...
#define SERVO_LEFT_MAX 48
#define SERVO_CENTER 76  // original 74
#define SERVO_RIGHT_MAX 117
...
#define __ADC_Read_Dist(_ADC, dataPoint, IR_data_raw_acc, obsDist, obsTick) ({ \
	HAL_ADC_Start(_ADC); \
	HAL_ADC_PollForConversion(_ADC,22000); \
	IR_data_raw_acc += HAL_ADC_GetValue(_ADC); \
	dataPoint = (dataPoint + 1) % IR_SAMPLE; \
	if (dataPoint == IR_SAMPLE - 1) { \
		obsDist = abs(IR_CONST_A / (IR_data_raw_acc / dataPoint - IR_CONST_B)); \
